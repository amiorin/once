...
import {
  ...
  describeReport,
  findContainerForHost,
  imageToRepositoryTag,
  matchingRepoDigest,
  onceCommandCheckArgs,
  parseOnceList,
  providerSummary,
  updateAvailable,
} from "../src/once/describe.js";
...
const ok = (out = ""): RunResult => ({ ok: true, exit: 0, out, err: "" });
const fail = (err: string): RunResult => ({
  ok: false,
  exit: 255,
  out: "",
  err,
...
it("docker container matching uses labels and names", () => {
  ...
  expect(
    findContainerForHost(containers, "www.example.com")?.Config?.Image,
  ).toBe("ghcr.io/org/app:latest");
});

it("digest selection and comparison", () => {
  expect(
    matchingRepoDigest("ghcr.io/org/app", [
...
it("describe remote command failure keeps compute running", () => {
  const runFn: RunFn = (args) => {
    ...
    if (eq(cmd, ["true"])) return ok();
    if (eq(cmd, onceCommandCheckArgs)) return ok();
    if (eq(cmd, ["sudo", "-n", "once", "list"])) return fail("once missing");
