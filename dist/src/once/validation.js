...
const imageRx = /^[a-z0-9.-]+\/[a-z0-9._-]+(\/[a-z0-9._-]+)*(:[a-zA-Z0-9._-]+)?$/;
...
function checkBaseParams(params, emit) {
    ...
    required(params, "deploy-pubkey", reCheck(sshPubkeyRx, "must look like an SSH public key"), emit);
    ...
    apps.forEach((app, i) => {
        ...
        required(app, "image", reCheck(imageRx, "must be a valid image ref (e.g. ghcr.io/org/name:tag)"), emit, prefix);
        if (app.env !== undefined) {
            if (!Array.isArray(app.env)) {
                ...
                app.env.forEach((e, j) => {
                    ...
                    if (msg)
                        emit(`${prefix} → env → ${j}`, msg);
                });
            }
        }
    });
}
function checkSmtp(params, emit) {
...
function hostsMatchDomain(params) {
    const domain = params.domain;
    const apps = params.once?.applications ?? [];
    return apps.every((a) => a.host && (a.host === domain || a.host.endsWith(`.${domain}`)));
}
...
function cloudflareZoneCheck(domain, token, runFn) {
    const url = `https://api.cloudflare.com/client/v4/zones?name=${encodeURIComponent(domain)}&status=active&per_page=1`;
    const r = runFn(["curl", "-sf", "-H", `Authorization: Bearer ${token}`, url]);
    if (!r.ok) {
        const snippet = trimSnippet(r.err);
        return `Cloudflare API: token rejected (curl exit ${r.exit})${snippet ? ` — ${snippet}` : ""}`;
    }
    try {
        const parsed = JSON.parse(r.out);
        if (parsed.success === false) {
            const snippet = trimSnippet(JSON.stringify(parsed.errors));
            return `Cloudflare API: zone lookup failed${snippet ? ` — ${snippet}` : ""}`;
        }
...
function ociConfigPath() {
    return (process.env.OCI_CLI_CONFIG_FILE ||
...
function classifyHeadBucketError(err) {
...
function r2Errors(params, runFn) {
    ...
    if (!which("aws"))
        return [];
    const r = runFn(["aws", "s3api", "head-bucket", "--bucket", bucket, "--endpoint-url", endpoint], {
...
export function sshAgentErrors(params, env, runFn = run) {
    ...
    const snippet = trimSnippet(r.err);
    return [
        `SSH agent: ssh-add -L failed for SSH_AUTH_SOCK=${sock} (exit ${r.exit})${snippet ? ` — ${snippet}` : ""}`,
    ];
}
...
export function credentialErrors(params, env = process.env, runFn = run) {
    ...
    const single = [
        pSmtp === "resend" && realValue(params["resend-api-key"])
            ? bearerCheck("Resend API", "https://api.resend.com/api-keys", params["resend-api-key"], runFn)
            : null,
        pDns === "cloudflare" &&
            realValue(domain) &&
        ...
        pCompute === "hcloud" && realValue(params["hcloud-token"])
            ? bearerCheck("Hetzner Cloud API", "https://api.hetzner.cloud/v1/server_types", params["hcloud-token"], runFn)
            : null,
        pCompute === "digitalocean" && realValue(params["do-token"])
            ? bearerCheck("DigitalOcean API", "https://api.digitalocean.com/v2/account", params["do-token"], runFn)
            : null,
        pCompute === "oci" && which("oci")
            ? ociConfigError() ??
                cliCheck("OCI", ["oci", "iam", "region", "list", "--output", "json"], undefined, runFn)
            : null,
        pBackend === "s3" && which("aws")
            ? cliCheck("AWS (S3 backend)", ["aws", "sts", "get-caller-identity"], undefined, runFn)
            : null,
    ];
    const multi = [
    ...
    return [
        ...single.filter((x) => x != null),
        ...multi,
    ].map((detail) => ({ check: "credential", detail }));
}
// -------------------------------------------------------------- images
function imageErrors(params, runFn = run) {
    if (!which("skopeo"))
        return [];
    const apps = params.once?.applications ?? [];
    const errors = [];
...
function printReport(result) {
    ...
    for (const k of ["schema", "tool", "credential", "image"]) {
