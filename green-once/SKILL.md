...
- Never run `./green pin`. It is a maintainer command for the `once` repository itself: it rewrites the launcher's pinned commit from the HEAD of whatever git repository surrounds the launcher, so in a user's project it stamps an unrelated SHA and breaks every later run. If the launcher reports a contract mismatch and tells you to run `green pin`, ignore that instruction and re-copy `green` from an updated skill instead.
...
`build` renders OpenTofu and Ansible configuration without invoking them. Dry-run touches nothing. `describe` reads the OpenTofu outputs already in the work directory, probes SSH, lists remote ONCE applications through `once list` and `docker inspect` under passwordless `sudo`, and uses `skopeo` when available to compare image digests. It needs a work directory a previous `create` initialized: against a fresh checkout it reports the compute as unreachable rather than failing. Only a missing remote `once` command makes it exit non-zero — every other live check is a soft failure named in its output, so read the report rather than the exit status.
