...
const deployScript = resolve(
  "src/resources/io/github/amiorin/once/tools/ansible/files/deploy",
);

const hasBb =
  spawnSync("bb", ["--version"], { encoding: "utf8" }).status === 0;
 ...
 * `once update`.
 */
function makeShim(listOutput: string): Shim {
...
function runDeploy(
  ...
  shim?: Shim,
): { exit: number; err: string } {
  const env: Record<string, string> = {
    ...(process.env as Record<string, string>),
    SSH_ORIGINAL_COMMAND: sshOriginalCommand ?? "",
  };
  if (shim) env.PATH = `${shim.dir}:${process.env.PATH ?? ""}`;
...
describe.skipIf(!hasBb)("deploy ForceCommand script", () => {
  ...
  it("rejects host not in once list", () => {
    ...
    const { exit, err } = runDeploy(
      "sudo once update bogus.example.com",
      shim,
    );
    expect(exit).toBe(1);
  ...
  it("parses the host list with ANSI escapes", () => {
    ...
    const { exit } = runDeploy(
      "sudo once update foo.bigconfig.space",
      shim,
    );
    expect(exit).toBe(0);
    expect(readFileSync(shim.log, "utf8")).toContain(
      "update foo.bigconfig.space",
    );
  });
});
