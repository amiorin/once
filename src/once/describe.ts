...
export type RunFn = (args: string[], opts?: RunOpts) => RunResult;
...
function resultDetail(label: string, r: RunResult): string {
  const snippet = trimSnippet(r.err) ?? trimSnippet(r.out);
  return `${label} failed (exit ${r.exit ?? -1})${
    snippet ? ` — ${snippet}` : ""
  }`;
}

function onceCommandNotFound(r: RunResult): boolean {
  const text = `${r.err ?? ""}\n${r.out ?? ""}`.toLowerCase();
  return (
    r.exit === 127 ||
...
function sshBaseArgs(compute: ComputeTarget): string[] {
  return [
    "ssh",
    "-o",
    "BatchMode=yes",
...
export function providerSummary(params: Record<string, any>): {
  compute: any;
  backend: any;
  smtp: any;
  dns: any;
...
export function computeTarget(params: Record<string, any>): ComputeTarget {
...
interface ComputeStatus extends ComputeTarget {
  running: boolean;
  detail: string;
}

function computeStatus(runFn: RunFn, params: Record<string, any>): ComputeStatus {
...
export interface OnceApp {
  host: string;
  status?: string;
}

/** Parse the `once list` output into hosts and statuses. */
export function parseOnceList(output: string): OnceApp[] {
  const result: OnceApp[] = [];
  for (const line of stripAnsi(output).split("\n")) {
    const m = line.match(hostStatusRx);
    if (m) {
      const app: OnceApp = { host: m[1] };
      if (m[2] && m[2].trim() !== "") app.status = m[2];
      result.push(app);
    }
  }
  return result;
...
export function imageToRepositoryTag(
  image: string | null | undefined,
): ParsedImage | undefined {
  const img = (image ?? "").toString().trim();
  if (img === "" || /^sha256:[A-Fa-f0-9]+$/.test(img)) return undefined;
...
export function matchingRepoDigest(
  repository: string,
...
export function updateAvailable(
  ...
  registryDigest: string | undefined | null,
): boolean | undefined {
  if (
    runningDigest &&
    runningDigest.trim() !== "" &&
...
function registryDigest(
  ...
  os: string,
  arch: string,
): { digest: string | null; detail?: string } {
  const args = ["skopeo", "inspect", "--no-tags"];
  if (!blank(os)) args.push("--override-os", os);
  if (!blank(arch)) args.push("--override-arch", arch);
...
function parseJsonVector(s: string): any[] {
  try {
...
function stringLeaves(x: any): string[] {
  if (x == null) return [];
  if (typeof x === "string") return [x];
  if (Array.isArray(x)) return x.flatMap(stringLeaves);
  if (typeof x === "object") {
...
function containerSearchText(container: any): string {
  const picked: Record<string, any> = {};
  for (const k of ["Name", "Config", "NetworkSettings"]) {
    if (k in container) picked[k] = container[k];
  }
  return stringLeaves(picked).join("\n").toLowerCase();
}

function containerForHost(host: string, container: any): boolean {
...
export function findContainerForHost(
  containers: any[],
  host: string,
): any | undefined {
...
function imageIdentifiers(containers: any[]): string[] {
  const ids = containers.flatMap((c) => [c.Image, c?.Config?.Image]);
  return [...new Set(ids.filter((x) => x != null && x !== ""))];
}

function imageInfoForContainer(
  imageInfos: any[],
  container: any,
  parsedImage: ParsedImage | undefined,
): any | undefined {
  ...
  return (
    imageInfos.find((i) => i.Id === imageId) ??
...
function applicationReport(
  ...
  if (registry?.detail) report.registryDetail = registry.detail;
  return report;
}

// -------------------------------------------------------------- remote discovery

interface RemoteAppsResult {
  ok: boolean;
  fatal?: boolean;
  detail?: string;
...
function remoteApplications(
  ...
  if (onceApps.length === 0) return { ok: true, applications: [] };
  ...
  if (imageIds.length === 0) {
    return {
      ok: true,
      applications: onceApps.map((a) =>
        applicationReport(runFn, containers, [], a),
      ),
    };
  }
  ...
  return {
    ok: true,
    applications: onceApps.map((a) =>
      applicationReport(runFn, containers, imageInfos, a),
    ),
  };
}
...
function resolveOnceOpts(
  opts: Opts,
  onceOptsFn: (opts: Opts) => Opts,
): { opts: Opts; detail?: string } {
  try {
...
export function describeReport(
  ...
  const appResult: RemoteAppsResult = compute.running
...
function present(x: unknown): string {
  return blank(x) ? "unknown" : String(x);
}

function updateLabel(x: boolean | null | undefined): string {
  if (x === true) return "yes";
  if (x === false) return "no";
  return "unknown";
}

function printReport(result: DescribeResult): void {
  const { profile, providers, compute, applications, applicationsError } =
    result;
  ...
  console.log(
    `  Status: ${compute.running ? "running" : "not reachable"}${
  ...
  if (applicationsError) {
    ...
    for (const app of applications) {
      ...
      console.log(
        `    digest: ${present(app.digest)}${
          app.digestSource === "image-id"
