...
- compute: `azure`, `aws`, `digitalocean`, `hcloud`, `yandex`, `oci`, `no-infra`
...
with `COLORS_PAR_`. OCI uses its profile, Azure uses the ambient Azure CLI session, and AWS compute and S3 use the AWS
