...
- Before delete, remind the user that `:compute-prevent-destroy` defaults to `true`. Authorize an intentional delete with `GREEN_PAR_COMPUTE_PREVENT_DESTROY=false` (or its `ONCE_PAR_*` alias) rather than editing desired state.
- Never edit `.once/`. Green, Red, and Blue may use it interchangeably, but two implementations must never run concurrently against the same state.
...
- profile name and working directory (default `.once`)
