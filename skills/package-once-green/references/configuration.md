...
**The machine key is generated by default.** When desired state carries no machine-key setting for the selected compute provider (`:compute-pubkey` on Yandex; `:digitalocean-ssh-keys`, `:hcloud-ssh-keys`, `:vultr-ssh-keys`; `:oci-ssh-authorized-keys` and the aws/azure/google equivalents), the first real `create` generates an ed25519 keypair named after the `profile` in `.ssh/` next to `colors.yml`, registers the public half with the provider — as a key resource named after the profile where the provider has one — and `delete` removes it after a successful destroy. An existing `.ssh/<profile>` without deployment state, or a provider-side key named after the profile outside the deployment's state, stops the create instead of being overwritten or adopted; the error names the recovery. See the SSH Keypair Standard (`workspace/standards/ssh-keypair.md`).

Setting the provider's machine-key value switches generation off and behaves exactly as before. `:compute-pubkey` is then installed for the `ubuntu` user through Yandex instance metadata (a present value must look like a public key); DigitalOcean, Hetzner, and Vultr reference keys already registered with them; OCI reads a local public-key file named by `:oci-ssh-authorized-keys`. In that mode keep the matching private half in `ssh-agent`.
