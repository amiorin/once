...
(deftest with-machine-key-fills-templates-deterministically
  (let [dir (tmp-project)]
    ...
    (testing "path providers get the absolute public key path"
      (let [out (sut/with-machine-key (opts-in dir :provider-compute "oci") false)]
        ...
        (is (str/ends-with? (:ssh-public-key-path out) "/.ssh/prod.pub"))
        (is (str/ends-with? (:ssh-private-key-path out) "/.ssh/prod"))))
    ...
    (testing "a build never reads .ssh/, even when it exists"
...
(deftest ensure-key-implements-the-create-matrix
  (testing "first create generates and the run proceeds"
    (let [dir (tmp-project)
          out (sut/ensure-key! (opts-in dir) (constantly nil) fake-keygen)]
  ...
  (testing "state without a key is lost access, not regeneration"
    (let [dir (tmp-project)
          out (sut/ensure-key! (opts-in dir) (constantly {:ip "1.2.3.4"})
                               fake-keygen)]
      (is (= 1 (:green/exit out)))
      (is (str/includes? (:green/err out) "lost the machine key"))
      (is (not (.exists (io/file dir ".ssh" "prod"))))))

  (testing "a key without state refuses rather than overwrites"
    (let [dir (tmp-project)
          keygen fake-keygen]
      (keygen ["ssh-keygen" (str (io/file dir ".ssh" "prod"))] {} 0)
  ...
  (testing "opt-out mode does nothing at all"
    (let [dir (tmp-project)
          opts (opts-in dir :vultr-ssh-keys "key-uuid")]
