...
(defn read-state
  ...
  else in the SDK does. A launch failure — the stage directory does not
  exist yet, as on a fresh clone's first create, or tofu is not on PATH — is
  reported by green's tofu runner as a failed exit (127) and so reaches here
  in that same shape, the way red and blue turn theirs into `StepError`.
  This function depends on that shape, as its red and blue twins depend on
  `red/tofu` and `blue.tofu` throwing `StepError`. A message-less step error
  reads as the fixed string `state read failed without a message`. Anything
  else propagates: a programmer defect in the reader must not read as \"no
  state\" and skip the switch guard."
  [opts reader]
